...
class EchoStateNetworkGPU:
    def __init__(self, input_size, reservoir_size, output_size, spectral_radius=0.95, 
        ...
        # Fix tensor initialization for CUDA compatibility
        ...
        self.W_out = None
        self.reservoir_state = torch.zeros((self.reservoir_size, 1), 
                                         device=self.device)
    ...
    def fit(self, inputs, targets, regularization=1e-8):
        ...
        # Add bias term
        states = torch.hstack((torch.ones(states.shape[0], 1, device=self.device), states))
        
        # Compute output weights using Ridge Regression (with GPU acceleration)
